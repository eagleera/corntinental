<template>
  <div class="card rounded-sm p-4 text-primary" v-if="room != null">
    <div class="row">
      <div class="col-8">
    ...
    <div class="row mt-4">
      <table class="table table-bordered">
        ...
        <tbody>
          <tr>
            <td>#6</td>
            <td v-for="(round, index) in round(1)" :key="index">{{ round.points }}</td>
          </tr>
          <tr>
            <td>#7</td>
...
export default {
  ...
  methods: {
    ...
    total(guest_id){
      return this.room.points.filter(point => point.guest_id == guest_id).reduce((prev,next) => prev + next.points,0);
    }
  },
  mounted() {
